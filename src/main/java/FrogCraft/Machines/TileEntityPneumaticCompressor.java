...
public class TileEntityPneumaticCompressor extends BaseIC2Machine implements ISidedInventory
{
	...
	public void onInventoryChanged()
	{
		if (inv[0] == null)
			return;
		rec = FrogCraft.Integration.GregTech.findImplosionRecipe(inv[0]);
	}
	...
	public void updateEntity()
	{
		...
		if (inv[0] != null && energy - 8 >= 0 & fully() && rec != null && (Integer) rec[2] <= inv[0].stackSize && (inv[1] == null || ((ItemStack) rec[0]).stackSize + inv[1].stackSize <= 64))
		{

			ItemStack out = (ItemStack) rec[0];
			gas_total = (Integer) rec[1];

			gas_total = gas_total * Frogcraft.rate_PneumaticCompressor;
	...
	int cosume_gas(int amount)
	{
		...
		TileEntity te[] =
		{ worldObj.getBlockTileEntity(xCoord + 2, yCoord - 3, zCoord), worldObj.getBlockTileEntity(xCoord - 2, yCoord - 3, zCoord), worldObj.getBlockTileEntity(xCoord, yCoord - 3, zCoord + 2), worldObj.getBlockTileEntity(xCoord, yCoord - 3, zCoord - 2) };
	...
	void boom()
	{
		if (Frogcraft.boom_PneumaticCompressor)
			worldObj.createExplosion(null, xCoord, yCoord + 1, zCoord, 0F, false);
	}

	void rndboom()
	{
		if (Frogcraft.rndboom_PneumaticCompressor)
		{
			if (worldObj.rand.nextInt(20) == 5)
				boom();
		}
	}
	...
	{
		return "tileentityPneumaticCompressor";
	}
